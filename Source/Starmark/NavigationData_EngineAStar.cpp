...
FPathFindingResult ANavigationData_EngineAStar::FindPath(const FNavAgentProperties& AgentProperties, const FPathFindingQuery& Query)
{
	...
	if (NavPath != nullptr) {
		...
		if ((Query.StartLocation - Query.EndLocation).IsNearlyZero()) {
			...
			AStar->WorldGridReference->GetGridCellForWorldPos(Query.StartLocation, StartPositionOnGrid);
			AStar->WorldGridReference->GetGridCellForWorldPos(Query.EndLocation, EndPositionOnGrid);
