...
float FGridPathFilter::IsTraversalAllowed(const int32 NodeA, const int32 NodeB) const
{
...
FPathFindingResult ARecastNavMesh_GraphAStar::FindPath(const FNavAgentProperties &AgentProperties, const FPathFindingQuery &Query)
{
	...
	const ARecastNavMesh *GraphAStarNavMesh 
	{
	...
	if (NavMesh && NavFilter) {
		...
		// Start of our code
			...
			{
				GraphAStarNavMesh->WorldGrid->CubeCoordinates.IndexOfByKey(StartCoord)
			};
			const int32 EndIndex
			{
				GraphAStarNavMesh->WorldGrid->CubeCoordinates.IndexOfByKey(EndCoord)
			};
			...
			//Internalize the pathfinder. Pass our GraphAStarNavMesh as a parameter so it can use the functions we implemented
			FGraphAStar<GraphAStarNavMesh> Pathfinder(*GraphAStarNavMesh);
...
// --------------------------------------------------
int32 ARecastNavMesh_GraphAStar::GetNeightbourCount(FNodeReference NodeReference) const
{
	return 8;
}

bool ARecastNavMesh_GraphAStar::IsValidReference(FNodeReference NodeReference) const
{
	return WorldGrid->GridCoordinates.IsValidIndex(NodeReference);
}

ARecastNavMesh_GraphAStar::FNodeReference
ARecastNavMesh_GraphAStar::GetNeighbour(const FNodeReference NodeReference, const int32 NeighbourIndex) const
{
	FWorldGridCoord Neighbour
	{
